...
export default function Search({ navigation }: ScreenProps) {
	...
	useEffect(() => {
		if (search.length >= 1) {
			LocalFileService.search(search).then(setResults).catch((_) => Alert.alert("Error", "Something went wrong!"));
	...
	return (
		<CustomSafeAreaView>
			<AnimatedHeader Component={StickyHeader} page={page} />
			<View flex={1} px={3}>
				<FlashList
					data={results}
					renderItem={({ item, index }) => (<SearchCard key={`${item.id}-${index}`} data={item} navigation={navigation} />)}
					keyExtractor={(item, index) => `${item.id}-${index}`}
					ListHeaderComponent={<SearchHeader search={search} setSearch={setSearch} />}
					ListEmptyComponent={
						<Flex w={width * 0.9} bg="transparent" alignItems="center" justifyContent="center" mx="auto" my={16}>
							<Box alignItems="center">
								<Icon as={MaterialIcons} name="folder" size={32} _dark={{ color: "muted.800" }} _light={{ color: "muted.300" }} />
								<Text fontSize={16} _dark={{ color: "muted.700" }} _light={{ color: "muted.400" }} mt={3}>
									No results...
								</Text>
							</Box>
						</Flex>
...
function SearchHeader({ search, setSearch, }: { search: string; setSearch: any; }) {
	return (
