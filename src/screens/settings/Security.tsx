			...
			<SettingsSection title="Security" hideTitle={true}>
				{supportsBio ? (
					<StaticSettings>
						...
						<Switch onToggle={() => appUtil.toggleBiometrics(!appState.useBiometrics)} value={appState.useBiometrics} />
					</StaticSettings>
				) : (
					<Text opacity={0.5} px={3} py={4}>
						Your device does not support FaceID or Fingerprint authentication, or you do not have any biometric authentication methods configured on it.
					</Text>
				)}
			</SettingsSection>
		</ScrollView>
