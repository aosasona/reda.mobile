			...
			<HStack space={2} alignItems="stretch">
				<Box w={width * 0.25} position="relative">
					...
					{meta?.currentIndex === index && (
						<VStack position="absolute" top={0} left={0} right={0} justifyContent="center" alignItems="center">
							<AspectRatio
								ratio={1}
								w="full"
								bg="dark.700"
				...
				<VStack
					...
					py={2}
				>
					<Heading fontSize={20} noOfLines={2}>{data?.title}</Heading>
					<HStack justifyContent="space-between" space={1}>
						{data?.author_name ? <Text opacity={0.9} fontWeight={600} noOfLines={1}>{data?.author_name[0]}</Text> : null}
						{data?.publish_year ? <Text opacity={0.3} fontSize={12} fontWeight={600}> {data?.publish_year[0]} </Text> : null}
					</HStack>
				</VStack>
			</HStack>
		</Pressable>
