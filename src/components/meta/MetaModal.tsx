...
import { ActionSheetProps } from "../../config/props";
...
export default function MetaModal({ functions, state }: MetaModalProps) {
	...
	function toggleStep() {
		setState((prevState) => ({ ...prevState, step: prevState.step === MetaModalSteps.ONE ? MetaModalSteps.TWO : MetaModalSteps.ONE }));
	}
			...
			{step === MetaModalSteps.TWO && (
				<Actionsheet.Content {...ActionSheetProps}>
					<PresenceTransition
						...
						animate={transitionAnimation}
					>
						<MetaPage state={{ data: meta?.current, file }} functions={{ handleComplete, toggleStep, handleModalDismiss }} />
					</PresenceTransition>
				</Actionsheet.Content>
			)}
		</Actionsheet>
