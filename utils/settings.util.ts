...
export default class SettingsUtil {
	...
	public loadSettings = async () => {
		...
		const useSinglePageLayout = !!(Number(await getAsyncStorage(ConfigKeys.SINGLE_PAGE_LAYOUT)));
		this.dispatch({
			type: GlobalActionType.LOAD_SETTINGS,
			payload: { fontSize, fontFamily, deleteFilesAfterImport, useSinglePageLayout }
		});
	}
	...
	public toggleSinglePageLayout = (value: boolean) => {
		const val = value ? 1 : 0
		this.dispatch({ type: GlobalActionType.SET_SINGLE_PAGE_LAYOUT_OPTION, payload: val })
	}
			...
			},
			{
				...
				onPress: () => {
					...
					Promise.all([delAll, clearDB])
						.then(() => Alert.alert("Success", "Data cleared! Restart app to see changes."))
						.catch(e => Alert.alert("Error", "An error occured"))
				},
				style: "destructive",
			},
		])
