...
export class RedaService {
	...
	static async getStarred(filter: QueryFilter = { limit: 25, sort_by: "name", sort_order: "DESC" }): Promise<CombinedFileResultType[] | null> {
	...
	static async getContinueReading(filter: QueryFilter = { limit: 25, sort_by: "created_at", sort_order: "DESC" }): Promise<CombinedFileResultType[]> {
		const { limit, sort_by, sort_order } = filter;
