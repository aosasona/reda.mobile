...
export default function MetaPage({ state, functions }: MetaPageProps) {
	...
	const getBookData = async () => {
		try {
			const bookSeed = data?.seed?.filter((seed: string) =>
				seed.startsWith("/books")
			)[0];
		...
		try {
			...
			const meta: MetadataModel = {
				...
				table_of_contents: JSON.stringify(
					bookData?.data?.table_of_contents || []
				),
				subjects:
					data?.subject_facet?.join(", ") ||
					bookData?.data?.subjects?.join(", ") ||
					"",
				first_publish_year:
					data?.first_publish_year ||
					bookData?.data?.first_publish_year ||
					data?.publish_year?.[0] ||
					0,
				...
				current_page: 1,
				total_pages:
					bookData?.data?.number_of_pages || data?.number_of_pages_median || 1,
			};
