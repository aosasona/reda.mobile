...
export default function PreviewHeader({ data, source, defaultSource, onPress, navigation }: PreviewHeaderProps) {
	...
	return <AspectRatio w="full" position="relative" ratio={1}>
		<ImageBackground source={source} defaultSource={defaultSource} resizeMode="cover">
			...
			<Flex flex={1} justifyContent="space-between" alignItems="flex-start" px={4} pb={4}>
				...
				<Box>
					<Box>
						<Heading color="white" fontSize={42} noOfLines={4} shadow={3}>{data?.name}</Heading>
						<Text color="white" fontSize={16} noOfLines={2} shadow={3} opacity={0.8} mt={2}>{data?.author}, {data?.first_publish_year || "2022"}</Text>
					</Box>
				</Box>
			</Flex>
